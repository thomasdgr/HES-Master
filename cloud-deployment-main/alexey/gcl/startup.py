...
def write_backend_sh():
    f = open("startup-script-backend.sh", "w")
    f.write("#!/bin/bash\n")
    f.write("cd /home/alexe/backend\n")
...
def write_frontend_sh(ip):
    ...
    f.write("#!/bin/bash\n")
    f.write("cd /home/alexe/frontend/\n")
    f.write("sed -i '1s/.*/const backaddr = \"" + ip +"\";/' script.js\n")
    f.write("http-server -p 8000\n")
    f.close()
...
def main(project, bucket, zone, instance_name, imagename, wait=True):
    ...
    operation = create_instance(compute, project, zone, instance_name[0], bucket, imagename[0],"startup-script-backend.sh")
    print("here")
    ...
    operation = create_instance(compute, project, zone, instance_name[1], bucket, imagename[1],"startup-script-frontend.sh")
    wait_for_operation(compute, project, zone, operation['name'])
...
if __name__ == '__main__':
    ...
    name = ["vm-backend-1", "vm-frontend-1"]
    imagename = ["image-back", "image-fro"]
